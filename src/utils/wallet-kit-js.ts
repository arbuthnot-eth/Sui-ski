...
export function generateWalletKitJs(config: WalletKitConfig): string {
    ...
    var SuiWalletKit = (function() {
        ...
        return {
	      ...
	      var __wkWindowWallets = [
	        ...
	        { check: function() {
	          ...
	          var sui = window.sui;
	          return (sui && !sui.isPhantom) ? sui : null;
	        }, name: 'Slush', icon: 'https://slush.app/favicon.ico' },
            ...
            try {
              if (wallet && wallet.name && !wallet.features && !wallet._raw && !wallet.__isPasskey) {
                ...
                if (realWallet) {
	              ...
	              if (__wkIsSubdomain()) {
	                __wkInitSignBridge();
	                var __bridgeWalletName = wallet && wallet.name ? String(wallet.name) : '';
	                var bridgeReady = __skiSignReady ? await __skiSignReady : false;
	                if (bridgeReady) {
	                  __wkSendWalletHintsToBridge([wallet]);
	                  var bridgeResult = await __wkConnectViaBridge(__bridgeWalletName);
	                  if (bridgeResult && bridgeResult.address) {
	                    var connectedWalletName = bridgeResult.walletName || __bridgeWalletName;
	                    initFromSession(bridgeResult.address, connectedWalletName);
	                    try {
	                      if (connectedWalletName) localStorage.setItem('sui_ski_last_wallet', connectedWalletName);
	                    } catch (_eLocal) {}
      ...
      function initFromSession(address, walletName) {
        ...
        try {
          if (__wkSessionWalletName) localStorage.setItem('sui_ski_last_wallet', __wkSessionWalletName);
        } catch (_eLocal) {}
        $connection.set({
          wallet: null,
          account: null,
