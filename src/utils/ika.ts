export interface IkaNetworkConfig {
	packages: {
		ikaPackage: string
...
/**
 * Resolve static Ika deployment data for a Sui network.
 * Defaults to testnet config for devnet since Ika does not target devnet yet.
 */
export function getIkaConfig(network: SupportedNetwork): IkaNetworkConfig {
	if (network === 'mainnet') {
		return {
			packages: {
	...
	// Default to testnet config (covers both testnet + devnet deployments)
	return {
		packages: {
