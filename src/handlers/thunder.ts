...
export const thunderRoutes = new Hono<ThunderEnv>()
...
function buildSystemPrompt(context: ThunderContext): string {
	const parts = [
...
thunderRoutes.get('/integrations', async (c) => {
	const env = c.get('env')
	...
	return c.json({
		...
		webMcp: {
			proxyEndpoint: '/api/thunder/webmcp',
			directEndpoint: '/mcp',
		...
		agents: {
			dispatchEndpoint: '/api/thunder/agents/dispatch',
...
thunderRoutes.all('/webmcp', async (c) => {
	const env = c.get('env')
	const req = c.req.raw
...
thunderRoutes.all('/webmcp/*', async (c) => {
	...
	const req = c.req.raw
	const reqUrl = new URL(req.url)
	const suffix = reqUrl.pathname.replace('/api/thunder/webmcp', '') || ''
...
thunderRoutes.post('/agents/dispatch', async (c) => {
	const req = c.req.raw
	let body: {
		agent?: string
		path?: string
	...
	const forwarded =
		agent === 'x402-register'
			? await x402RegisterRoutes.fetch(forwardReq, c.env, c.executionCtx)
			: await agentGraceVaultRoutes.fetch(forwardReq, c.env, c.executionCtx)
...
thunderRoutes.post('/agents/webmcp/tool', async (c) => {
	const env = c.get('env')
	const req = c.req.raw
	const reqUrl = new URL(req.url)
	let body: {
...
thunderRoutes.post('/chat', async (c) => {
	const env = c.get('env')
	...
	let body: ThunderRequest
	try {
		body = await c.req.json<ThunderRequest>()
	} catch {
		return c.json({ error: 'Invalid request body', code: 'INVALID_BODY' }, 400)
	}
	...
	const tab = (await getCached<ThunderTab>(tabKey(address))) || {
		messages: 0,
		costMist: '0',
		settledMist: '0',
	...
	if (detectDiceIntent(message)) {
		...
		const commit: ThunderDiceCommit = {
			commitHash,
			serverEntropy,
			timestamp: Date.now(),
		}
		...
		const response: ThunderResponse = {
	...
	const pendingDice = await getCached<ThunderDiceCommit>(diceKey(address))
	if (pendingDice && Date.now() - pendingDice.timestamp < DICE_COMMIT_TTL_SECONDS * 1000) {
		...
		const reveal: ThunderDiceReveal = {
			result: resultNum,
...
thunderRoutes.get('/session', async (c) => {
	const session = c.get('session')
	const address = session.address
	if (!address) {
		return c.json({
...
thunderRoutes.post('/settle', async (c) => {
	const session = c.get('session')
	const address = session.address
	if (!address) {
		return c.json({ error: 'Wallet not connected', code: 'AUTH_REQUIRED' }, 401)
...
thunderRoutes.post('/dice', async (c) => {
	const session = c.get('session')
	const address = session.address
	if (!address) {
		return c.json({ error: 'Wallet not connected', code: 'AUTH_REQUIRED' }, 401)
...
thunderRoutes.post('/dice/reveal', async (c) => {
	const session = c.get('session')
	const address = session.address
	if (!address) {
		return c.json({ error: 'Wallet not connected', code: 'AUTH_REQUIRED' }, 401)
	...
	const commit = await getCached<ThunderDiceCommit>(diceKey(address))
	if (!commit) {
...
thunderRoutes.get('/channels', async (c) => {
	return c.json({
		server: {
			id: 'decentralized',
...
thunderRoutes.post('/channels', async (c) => {
	return c.json({
...
thunderRoutes.delete('/channels/:id', async (c) => {
	return c.json({
		error: 'Use client-side @mysten/messaging SDK',
		code: 'CLIENT_SIDE_ONLY',
...
thunderRoutes.post('/channels/:id/mute', async (c) => {
	return c.json({
		error: 'Use client-side @mysten/messaging SDK',
		code: 'CLIENT_SIDE_ONLY',
...
thunderRoutes.get('/channels/:id/messages', async (c) => {
	return c.json({
		error: 'Use client-side @mysten/messaging SDK',
		code: 'CLIENT_SIDE_ONLY',
...
thunderRoutes.delete('/channels/:id/messages/:messageId', async (c) => {
	return c.json({
		error: 'Use client-side @mysten/messaging SDK',
		code: 'CLIENT_SIDE_ONLY',
...
thunderRoutes.post('/channels/:id/messages', async (c) => {
	return c.json({
		error: 'Use client-side @mysten/messaging SDK',
		code: 'CLIENT_SIDE_ONLY',
...
function generateSuggestions(context: ThunderContext, aiReply: string): string[] {
